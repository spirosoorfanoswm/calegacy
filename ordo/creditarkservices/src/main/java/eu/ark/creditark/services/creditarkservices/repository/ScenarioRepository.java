...
public class ScenarioRepository {
    ...
    private static String get_scenario_parameters;
    private static String setSystemScenarioReady;
    private static List<String> deleteDefaultScenarios = new ArrayList<>();
    ...
    static {
        ...
        get_scenario_parameters = "select * from d_scenario where scenario_id=?";
        deleteDefaultScenarios.add("delete from d_prospect_results where scenario_id in (select scenario_id from d_system_scenario where scenario_id not IN (select max(scenario_id) from d_system_scenario))");
        deleteDefaultScenarios.add("delete from d_scenario_customer where scenario_id in (select scenario_id from d_system_scenario where scenario_id not IN (select max(scenario_id) from d_system_scenario))");
        deleteDefaultScenarios.add("delete from d_scenario_portfolio where scenario_id in (select scenario_id from d_system_scenario where scenario_id not IN (select max(scenario_id) from d_system_scenario))");
        deleteDefaultScenarios.add("delete from d_scenario_prospect where scenario_id in (select scenario_id from d_system_scenario where scenario_id not IN (select max(scenario_id) from d_system_scenario))");
        deleteDefaultScenarios.add("delete from d_customer_results where scenario_id in (select scenario_id from d_system_scenario where scenario_id not IN (select max(scenario_id) from d_system_scenario))");
        deleteDefaultScenarios.add("delete from d_scenario where scenario_id in (select scenario_id from d_system_scenario where scenario_id not IN (select max(scenario_id) from d_system_scenario))");
        deleteDefaultScenarios.add("delete from d_system_scenario where scenario_id in (select scenario_id from d_system_scenario where scenario_id not IN (select max(scenario_id) from d_system_scenario))");
        setSystemScenarioReady = "UPDATE d_system_scenario set ready=true where scenario_id=?";
    }
    ...
    public Map<Integer, Scenario> getScenarios(ScenarioInputDto input,
        ...
        logger.info("getScenarios {} {} {} {} {}", schema, reduce, scenarioId, contextInfo, input);
    ...
    public ScenarioParams fetchScenarioParameters(int scenarioId, String schema) {
        logger.info("fetchScenarioParameters {} {}", scenarioId, schema);
        PreparedStatement st = null;
        ResultSet rows = null;
        try (Connection conn = jdbcTemplate.getDataSource().getConnection()) {
            conn.prepareStatement("SET search_path TO "+schema+", public, core, ext").execute();
            st = conn.prepareStatement(get_scenario_parameters);
            st.setInt(1, scenarioId);
            st.execute();
            rows = st.getResultSet();
            rows.next();
            ScenarioParams par = (new ScenarioParams.Builder())
                    .withScenarioId(scenarioId)
                    .withSnapshotDate(rows.getDate("snapshot_date"))
                    .withOwner(rows.getString("current_owner"))
                    .withContextId(rows.getInt("context_id"))
                    .withCreditAmount(rows.getDouble("credit_amount"))
                    .withDesc(rows.getString("descr"))
                    .withWacc(rows.getDouble("wacc"))
                    .withRaroc(rows.getDouble("raroc"))
                    .withMarginChange(rows.getDouble("margin_change"))
                    .withMaxLimitGrowth(rows.getDouble("max_limit_growth"))
                    .withMaxLimitReduction(rows.getDouble("max_limit_reduction"))
                    .withSignificantDigits(rows.getInt("significant_digits"))

                    .withMinLimit(rows.getInt("min_limit"))
                    .withWorstAcceptableScore(rows.getString("worst_accepted_score"))
                    .withMinLimitGrowth(rows.getDouble("min_limit_growth"))
                    .withMinLimitReduction(rows.getDouble("min_limit_reduction"))
                    .withMinAcceptedLimitUse(rows.getDouble("min_accepted_limit_use"))
                    .withMaxDso(rows.getInt("max_dso"))
                    .withMinAcceptedRwMargin(rows.getDouble("min_accepted_rw_margin"))
                    .withSalesChange(rows.getDouble("sales_change"))
                    .withPdChange(rows.getDouble("pd_change"))
                    .withMinLimitPct(rows.getDouble("min_limit_pct"))
                    .build();
              return par;
    ...
    public ScenarioStatistics getScenarioStatistics(int scenarioId, short contextId, String schema) throws DatabaseConnectionException {
        logger.info("getScenarioStatistics {} {} {}", scenarioId, contextId, schema);
        PreparedStatement st = null;
        ResultSet rows = null;
    ...
    public void deletePreviousDefaultScenarios(int contextId) {
        logger.info("deletePreviousDefaultScenarios start {}", contextId);
        CallableStatement cs = null;
        try (Connection conn = jdbcTemplate.getDataSource().getConnection()) {
            String schema = utilService.getSchema((short)contextId);
            conn.prepareStatement("SET search_path TO "+schema+", public, core, ext").execute();
            for(String sqlDele: deleteDefaultScenarios) {
                cs = conn.prepareCall(sqlDele);
                cs.execute();
            }

        }  catch (Exception e) {
            logger.error("deletePreviousDefaultScenarios  {}", e);

        } finally {
            logger.info("deletePreviousDefaultScenarios finish {}", contextId);
            DbUtils.close(cs);
        }
    }
    public void advertiseSystemScenario(int scenarioId) throws SQLException {
        logger.info("advertiseSystemScenario {}", scenarioId);
        try (Connection conn = jdbcTemplate.getDataSource().getConnection()) {
    ...
    public void saveScenario(int scenarioId, ScenarioBus bus, String schema)
            throws DatabaseConnectionException {
        logger.info("saveScenario {} {}", scenarioId, schema);
        CallableStatement cs = null;
        try (Connection conn = jdbcTemplate.getDataSource().getConnection()) {
    ...
    private void saveScenarioParams(Connection conn, int scenarioId, ScenarioBus bus)
            throws DatabaseConnectionException {
        logger.info("saveScenarioParams {}", scenarioId);
        CallableStatement cs = null;
        try  {
            ...
            logger.error("saveScenarioParams scenarioId {} {}" , scenarioId, e);
    ...
    public Parameters attachOptimizerParams(int scenarioId, String schema) throws DatabaseConnectionException {
        ...
        logger.info("attachOptimizerParams {} {}", scenarioId, schema);
        try (Connection conn = jdbcTemplate.getDataSource().getConnection()) {
            conn.prepareStatement("SET search_path TO "+schema+", public, core, ext").execute();
    ...
    public void setSystemScenarioReady(int scenarioId, String schema) throws DatabaseConnectionException {
        logger.info("setSystemScenarioReady start {}", scenarioId);
        PreparedStatement st = null;
        try (Connection conn = jdbcTemplate.getDataSource().getConnection()) {
            jdbcTemplate.update(new PreparedStatementCreator() {
                @Override
                public PreparedStatement createPreparedStatement(Connection connection) throws SQLException {
                    PreparedStatement stmt = conn
                            .prepareStatement(setSystemScenarioReady);
                    stmt.setInt(1, scenarioId);
                    return stmt;
                }
            });
        } catch (Exception e) {
            logger.error("setSystemScenarioReady error {} {}", scenarioId, e);

        } finally {
            logger.info("setSystemScenarioReady ready {}", scenarioId);
        }
    }
    public ScenarioInfo getScenarioInfo(int scenarioId, Parameters parameters, String schema, ScenarioInfo scenarioInfo)

            throws DatabaseConnectionException {
        logger.info("getScenarioInfo {} {}", scenarioId, schema);
        PreparedStatement st = null;
        ResultSet rows = null;
        try (Connection conn = jdbcTemplate.getDataSource().getConnection()) {
            conn.prepareStatement("SET search_path TO "+schema+", public, core, ext").execute();
            ...
            logger.error("getScenarioInfo: {} {}", scenarioId, e);
    ...
    public void deleteScenarioResults(int scenarioId, String schema) throws DatabaseConnectionException {
        logger.error("deleteScenarioResults: {} {}", scenarioId, schema);
        PreparedStatement st = null;
        try (Connection conn = jdbcTemplate.getDataSource().getConnection()) {
            conn.prepareStatement("SET search_path TO "+schema+", public, core, ext").execute();
    ...
    public StringBuilder saveScenarioResults(int scenarioId,
                                    Parameters parameters,
                                    ScenarioInfo scenarioInfo,
                                    String schema, String user)
            throws DatabaseConnectionException {
        logger.info("saveScenarioResults {}", scenarioId);
