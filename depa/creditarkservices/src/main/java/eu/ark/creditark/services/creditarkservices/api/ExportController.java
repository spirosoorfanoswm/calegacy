...
public class ExportController {
    ...
    @GetMapping(path = "/downloadreportsfile")
    public ResponseEntity downloadReports() throws IOException {
        logger.info("handleDownloadReportsResponse");
    ...
    public ResponseEntity<ExportFileEmptyResponse> statistics(Authentication authentication,
        ...
        if(!resp.isExists()) {
            ...
            return ResponseEntity.ok(exportService.clienteleStatisticsReport( businessService.clienteleStatistics(
    ...
    public ResponseEntity<ExportFileEmptyResponse> customer(Authentication authentication,
            ...
            throws DatabaseConnectionException, ParseException, IOException, DRException {
        logger.info("customers user:{} customerId:{} snapshotDate:{} period:{} contextId:{} scenarioId:{}", (null == authentication || null == authentication.getName())?
                        env.getProperty("ca.default.user"):authentication.getName(), customerId, snapshotDate, period, contextId, scenarioId);

        StringBuilder filename = new StringBuilder("customer_");
        filename.append(customerId).append("_")
                .append(snapshotDate).append("_")
                .append(period).append("_")
                .append(scenarioId).append("_")
                .append(contextId).append(".pdf");
        ExportFileEmptyResponse resp = exportService.fileExists(filename.toString());
        if(!resp.isExists()) {
            resp.setExists(true);
            resp.setFileName(filename.toString());
            return ResponseEntity.ok(exportService.customerDetailsReport( businessService.customer(new CustomerInputDto(customerId, DateUtils.stringToDate(
                                        snapshotDate,
